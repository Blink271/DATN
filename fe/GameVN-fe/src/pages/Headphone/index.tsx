...
import { Product, HeadphoneDetails } from '../../types'
...
const HeadPhonePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchMice = async () => {
      try {
        setLoading(true)
        const response = await axios.get<Product[]>(`${API_URL}/headphone`)
        setHeadPhone(response.data)
      } catch (err) {
        setError('Không thể tải danh sách tai nghe')
  ...
  if (loading) {
    return <div className='text-center py-8'>Đang tải danh sách tai nghe...</div>
  }
  ...
  if (headphone.length === 0) {
    return <div className='text-center py-8'>Không có sản phẩm tai nghe nào</div>
  }
  ...
  return (
    <div className='container mx-auto p-4'>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6'>
        {headphone.slice(0, visibleCount).map((product) => {
          const details = product.details as HeadphoneDetails
          return (
            <Link to={`/product/headphone/${product._id}`} key={product._id}>
              <div className='border rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow hover:cursor-pointer h-full flex flex-col'>
                ...
                <div className='space-y-1 text-xs text-gray-600 mb-2'>
                  <div className='flex items-center'>
                    ...
                    <span>Kiểu: {details.driver_size}</span>
                  </div>
                  <div className='flex items-center'>
                    <span className='mr-1'>•</span>
                  ...
                  <div className='flex items-center'>
                    <span className='mr-1'>•</span>
                    <span>{details.microphone ? 'Có Mic' : 'Không Mic'}</span>
                  </div>
                </div>
